...
public class BulletC extends TanksC{

	int host;//Dan tanks bot la 1, dan dong minh la 2.
	int lv=0;
	int maDan=0;
	public BulletC(int x, int y, int huong, int type, int wi, int le, int host) {
	...
	public boolean Destroy_Tanks()
	{
		...
		if(this.huong == 3)
			for(TanksC o:TanksC.O_Tanks)
//			for(Iterator<Tanks> o = Tanks.O_Tanks.iterator(); o.hasNext();)	
			{
				if(o.live ==true &&o.type == dich)
