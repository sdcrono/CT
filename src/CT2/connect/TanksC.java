...
public class TanksC {
	...
	static public ArrayList<TanksC> O_Tanks = new ArrayList<TanksC>(); //Mang chua tat ca doi tuong xe tang da tao ra.
	static public ArrayList<BulletC> O_Bullets = new ArrayList<BulletC>();
...
//	boolean flash_player = false;	
	...
	//Ham khoi tao tanks.
	public TanksC(int x, int y, int huong, int type, int size)
	{
		
	}
	
	//Ham khoi tao vat the bat ky.
	public TanksC(int x, int y, int huong, int type, int wi, int le, int host)
	{
	...
	{
		MapC.DelRect(this.X, this.Y);
	}
	
	// Luu vi tri Tank vao Map.
	protected void Move()
	{
		MapC.SetRect(this.X, this.Y, this.size, this.size, this.type);
	}
	...
	public void Shoot()
	{
		if(this.t_delay_shoot == 0 && this.live )
		{
			...
			BulletC bullet = null ;
			if(this.huong == 0)
				bullet= new BulletC(this.X+13, this.Y-1, this.huong, 1, 4, 10, this.type);
			else
			if(this.huong == 1)
				bullet = new BulletC(this.X+30, this.Y+13, this.huong, 1, 10, 4, this.type);
			else
			if(this.huong == 2)
				bullet = new BulletC(this.X+13, this.Y+30, this.huong, 1, 4, 10, this.type);
			else
			if(this.huong == 3)
				bullet = new BulletC(this.X-1, this.Y+13, this.huong, 1, 10, 4, this.type);
			if (bullet != null)
			{
				audio.Tank_shoot();
				TanksC.O_Bullets.add(bullet);
			}	
...
//			audio.Girl_die();
			...
			if(TanksPlayerC.streaktolive >0)
			{
				...
				TanksPlayerC.streaktolive = 1000;
//				while (TanksPlayer.delay_voice > 0)
//				{
//					
//				}
				audio.Chain_streak();
				TanksPlayerC.streaktostack++;
...
//			System.out.println(random_voice);
			TanksC.player --;
			if(TanksC.player == 1 && random_voice ==1)
			{
			...
			if(TanksPlayerC.score > 3)
				audio.Shut_down();
			TanksPlayerC.streaktocount = 0;
			TanksPlayerC.streaktostack = 0;
//			System.out.println(Tanks.player);
	...
	public boolean True_Move(int x, int y, int h)
	{
		...
		if(h == 0 || h == 2)
			if(MapC.map[x][y].Get() != 0 || MapC.map[x+14][y].Get() != 0 ||MapC.map[x+29][y].Get() != 0)
				return false;
		
		if(h == 1 || h == 3)
			if(MapC.map[x][y].Get() != 0 || MapC.map[x][y+14].Get() != 0 || MapC.map[x][y+29].Get() != 0)
				return false;
