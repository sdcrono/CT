...
public class Bulletv2 extends Bullet{

	int host;//Dan tanks bot la 1, dan dong minh la 2.
	int sequenceNumber;
	int lv=0;
	public static int trungDongThoi = 0;
	public Bulletv2(int x, int y, int huong, int type, int wi, int le, int host, int sequenceNumber) {
			...
			if(this.huong == 0)
				for(Playout o:Playout.O_Playout)
				{
					...
					if( ((this.X>=o.x && this.X<=(o.x+24)) || (this.X+this.width -1>=o.x && this.X+this.width -1<=(o.x+24))) && (o.y +24 == this.Y ))
					{
						...
						if(Bulletv2.trungDongThoi%2 == 0)
						{
							changeDirection();
							Bulletv2.trungDongThoi++;
						}
						else
						{
							changeDirection();
