...
public class Bulletv4 extends Bullet{

	int host;//Dan tanks bot la 1, dan dong minh la 2.
	int sequenceNumber;
	int lv=0;
	public static int trungDongThoi = 0;
	public Bulletv4(int x, int y, int huong, int type, int wi, int le, int host, int sequenceNumber) {
		...
		boolean t= false;
			...
			if(this.huong == 0)
				for(Playout o:Playout.O_Playout)
				{
					...
					{
						if(Bulletv4.trungDongThoi%2 == 0)
						{
							changeDirection();
							Bulletv4.trungDongThoi++;
						}
						else
						{
							changeDirection();
