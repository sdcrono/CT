...
public class MapClient {
	...
	public static class Screen extends Frame implements KeyListener
	{
		...
		void drawIntro(Graphics g)
		{
			...
			String textU = "^";
			String textLDR = "<  v  >";
			String textScore = "Score: "  +  String.valueOf(TanksPlayerClient.score);
...
//		    }
			...
			for(int i = 1; i<=10; i++)
			{
				if(TanksPlayerClient.score >  score_sv[i])
				{
					t1 = score_sv[i];
					...
					score_sv[i] = TanksPlayerClient.score;
					bienGiuViTri = i;
					break;
				}
			}
...
//				score_sv[n] = Integer.parseInt(in.next());
		...
		public void keyPressed(KeyEvent e) {
			...
			{
				CoTClient.Co_Op = false;
				CoTClient.pause = false;
				CoTClient.Intro = false;
			}
			if(e.getKeyCode()== KeyEvent.VK_2 )
			{
				CoTClient.Co_Op = true;
				CoTClient.pause = false;
				CoTClient.Intro = false;
			}

			if(e.getKeyCode()== KeyEvent.VK_ENTER && !CoTClient.pause && !CoTClient.Intro)
				CoTClient.pause = true;
			else
				if(e.getKeyCode()== KeyEvent.VK_ENTER && !CoTClient.Intro)
					CoTClient.pause = false;
