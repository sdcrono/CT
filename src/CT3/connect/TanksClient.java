...
public class TanksClient {
	...
	static public ArrayList<TanksClient> O_Tanks = new ArrayList<TanksClient>(); //Mang chua tat ca doi tuong xe tang da tao ra.
	static public ArrayList<BulletClient> O_Bullets = new ArrayList<BulletClient>();
	...
	//Ham khoi tao tanks.
	public TanksClient(int x, int y, int huong, int type, int size)
	{
		
	}
	
	//Ham khoi tao vat the bat ky.
	public TanksClient(int x, int y, int huong, int type, int wi, int le, int host)
	{
...
//	
	...
	{
		MapClient.DelRect(this.X, this.Y);
	}
	
	// Luu vi tri Tank vao Map.
	protected void Move()
	{
		MapClient.SetRect(this.X, this.Y, this.size, this.size, this.type);
...
//		
//
//			BulletClient bullet = null ;
//			if(this.huong == 0)
//				bullet= new BulletClient(this.X+13, this.Y-1, this.huong, 1, 10, 4, this.type);
//			else
//			if(this.huong == 1)
//				bullet = new BulletClient(this.X+30, this.Y+13, this.huong, 1, 10, 4, this.type);
//			else
//			if(this.huong == 2)
//				bullet = new BulletClient(this.X+13, this.Y+30, this.huong, 1, 10, 4, this.type);
//			else
//			if(this.huong == 3)
//				bullet = new BulletClient(this.X-1, this.Y+13, this.huong, 1, 10, 4, this.type);
//			if (bullet != null)
//			{
//				audio.Tank_shoot();
//				TanksClient.O_Bullets.add(bullet);
//			}	
//			
//			
//					
...
//	
	...
	public void Del_Tank()
	{
		...
		{
			TanksBotClient.bot-- ;
			TanksPlayerClient.score++ ;
			TanksClient.new_tank ++; 
		}
...
//		
//		if(h == 0 || h == 2)
//			if(MapClient.map[x][y].Get() != 0 || MapClient.map[x+14][y].Get() != 0 ||MapClient.map[x+29][y].Get() != 0)
//				return false;
//		
//		if(h == 1 || h == 3)
//			if(MapClient.map[x][y].Get() != 0 || MapClient.map[x][y+14].Get() != 0 || MapClient.map[x][y+29].Get() != 0)
//				return false;
//		
////		if(h == 2)
